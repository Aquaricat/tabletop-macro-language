  ...
  return new Promise(async (resolve, reject) => {
    ...
    step.args.forEach((arg) => {
      if (typeof arg.Roll !== 'undefined') {
        ...
        } else if (typeof rollArg.XF !== 'undefined') {
          command += `xf${getArgValue(rollArg.XF)}`
        } else if (typeof rollArg.XO !== 'undefined') {
          command += `xf${getArgValue(rollArg.XO)}`
    ...
    try {
      ...
      const formattedRoll = Object.assign(
        {}, apiResponse.roll, runAs, { to, is_test: executor._test }
      )
  ...
  return new Promise(async (resolve, reject) => {
    if (executor.prompt) {
      ...
      step.args.forEach((arg) => {
        ...
        if (typeof arg[op].options === 'object') {
      ...
      try {
        ...
        const result = defaultResult === undefined
